...
public class TelemetryJobServiceCN extends JobService {
    private static final String LOG_TAG = "TelemetryJobServiceCN";
    ...
    private org.mozilla.focus.telemetry.TelemetryJobServiceCN.UploadPingsTask uploadTask;

    @Override
    public boolean onStartJob(JobParameters params) {
        uploadTask = new org.mozilla.focus.telemetry.TelemetryJobServiceCN.UploadPingsTask();
    ...
    @VisibleForTesting public void uploadPingsInBackground(AsyncTask task, JobParameters parameters) {
        final TelemetryCN telemetry = TelemetryHolderCN.get();
    ...
    private boolean performPingUpload(TelemetryCN telemetry, final String pingType) {
        final TelemetryConfiguration configuration = telemetry.getConfiguration();
        final TelemetryStorage storage = telemetry.getStorage();
        final TelemetryClientCN client = telemetry.getClient();
        ...
        return storage.process(pingType, new TelemetryStorage.TelemetryStorageCallback() {
            ...
            public boolean onTelemetryPingLoaded(String path, String serializedPing) {
                Log.e("HttpCNTracking","performingPingUpload JobServiceCN");
                return !hasReachedUploadLimit(configuration, pingType)
                        && client.uploadPing(configuration, path, serializedPing,pingType)
